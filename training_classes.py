...
def copy_or_not(w, l2, english_d, ed):
    ...
    if len(en_single_word) == 0:
        ...
        if ed2_de >= ed2_en:
            sys.stderr.write('english ed was lower, guess:' + w + ', correction:' + en_single_word[
                0] + ', german context:' + l2 + '\n')
            return en_single_word[0], en_single_word[0], 'en'
        else:
            sys.stderr.write('german ed was lower, guess:' + w + ',en correction:' + en_single_word[
                0] + ', german context:' + l2 + '\n')
            return '__COPY__', l2, 'de'
...
class Guess(dict):
    def __init__(self, id, guess, revealed, l2_word):
        ...
        if self.guess.strip() == '':
            self.guess = '__blank__'
        elif self.guess.lower().strip() == self.l2_word.lower().strip():
            self.guess = '__copy__'
        else:
            pass
        self.id = id
...
class SimpleNode(dict):
    def __init__(self, sent_id, id, l2_word, position, lang, l1_parent):
        ...
        self.id = id
        self.l2_word = l2_word
        # self.l1_parent = l1_parent.lower()
        if l1_parent.strip() == '':
            self.l1_parent = '__blank__'
        else:
            l1_len, l1_single_word = sorted([(len(h), h) for h in l1_parent.split()]).pop()
            self.l1_parent = l1_single_word.lower()
